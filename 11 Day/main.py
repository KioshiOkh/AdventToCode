...
def processMonkeys():
    ...
    with open('input', 'r') as f:
        for line in f.read().split('\n'):
            if 'Monkey' in line:  
                monkeyNumber = int(line[7])
                mDict[monkeyNumber] = {}
            elif 'Starting' in line:
                monkeyItems = [int(s) for s in re.findall(r'-?\d+\.?\d*', line)]
                mDict[monkeyNumber]['Items'] = monkeyItems
            elif 'Operation' in line:
                stressMonkey = line.partition('old ')[2]
                mDict[monkeyNumber]['Stress'] = stressMonkey
            elif 'Test' in line:
                divideMonkey = re.findall(r'-?\d+\.?\d*', line)
                mDict[monkeyNumber]['Divide'] = int(divideMonkey[0]) 
            elif 'true' in line:
                throwMonkey = re.findall(r'-?\d+\.?\d*', line)
                mDict[monkeyNumber]['True'] = int(throwMonkey[0])
            elif 'false' in line:
                throwMonkey = re.findall(r'-?\d+\.?\d*', line)
                mDict[monkeyNumber]['False'] = int(throwMonkey[0])
    return mDict
